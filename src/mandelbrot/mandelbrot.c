...
int mandelbrot(int Px, int Py) 
{
    double x0 = (double)Px/(double)INIT_WIDTH - 1;
    double y0 = (double)Py/(double)INIT_HEIGHT;
    int n = 0;
    double tmp;
    double x = 0, y = 0;
...
void draw_square(SDL_Surface * surface, int x, int y, int m)
{
    ...
    Uint8 v = 255 - (m * 255 / MAX_ITER);
    SDL_FillRect(surface, rect, SDL_MapRGB(surface->format,v,v,v));
    free(rect);
}
...
{
    // Draws the fractal
    SDL_Surface * surface = SDL_CreateRGBSurface(0,INIT_WIDTH,INIT_HEIGHT,32,0,0,0,0);
    draw(renderer, surface, INIT_WIDTH, INIT_HEIGHT);
...
int main()
{
    ...
    // Creates a window.
    SDL_Window* window = SDL_CreateWindow("Mandelbrot", 0, 0, INIT_WIDTH, INIT_HEIGHT, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
    if (window == NULL)
        errx(EXIT_FAILURE, "%s", SDL_GetError());
